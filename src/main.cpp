...
#if defined(NDEBUG)
# error "HawaiiCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "HawaiiCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    int64_t nSubsidy = 0 * COIN;
 	...
 	if (nHeight == 1){nSubsidy = 30000 * COIN;}
 	else if (nHeight > 1 && nHeight <= 501){nSubsidy = .01 * COIN;}
 	else if (nHeight > 501 && nHeight <= 18000){nSubsidy = 5 * COIN;}
 	else if (nHeight > 18000){nSubsidy = .05 * COIN;}
...
{
    RenameThread("hawaiicoin-loadblk");
